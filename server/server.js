...
app.post("/uploadFace", upload.single("myImage"), function(req, res) {
  client
    .faceDetection(req.file.buffer)
    .then(results => {
      console.log(results);
      const faceDetection = results[0].faceAnnotations;
      var identificationF = { breed: faceDetection[0].joyLikelihood};
      console.log(faceDetection.joyLikelihood);
      res.json(faceDetection[0].joyLikelihood);
    })
    .catch(err => {
      console.error("ERROR:", err);
    });
